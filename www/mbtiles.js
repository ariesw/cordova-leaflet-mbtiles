cordova.define("cordova-leaflet-mbtiles.CordovaLeafletMBTiles", function(require, exports, module) {
...
if (!window.hasOwnProperty('L')) {
    ...
    L.GridLayer.MBTilesLayer = L.TileLayer.extend({
        ...
        getTileFromDb: function (data, db) {
            var that = this;

            return new Promise(function promise(resolve, reject) {
                db.executeSql('SELECT base64(tile_data) as tile_data FROM tiles WHERE zoom_level = ? AND tile_column = ? AND tile_row = ?',
                    [data.z, data.x, data['-y']],
                    function success(result) {
                        if (result.rows.length > 0) {
                            try {
								resolve({'dataurl': 'data:image/png;base64,' + result.rows.item(0).tile_data});
                            } catch (e) {
        ...
        createTile: function (coords, done) {
            ...
            if (!that.db) {
                ...
                var promises = [that.getTileFromDb(data, that.db)];
                ...
                Promise.all(promises.map(reflect)).then(function (results) {
                    var error = null; //Keep track of error if needed to pass to done
